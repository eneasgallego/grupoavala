<?php 
	...
	$captcha = imagecreatetruecolor(80,35);
	$background_color = imagecolorallocate($captcha, 17,30,67);
	imagefill($captcha, 0, 0, $background_color);
	
	#draw some lines
	for($i=0;$i<10;$i++){
		$color = rand(48,96);
		imageline($captcha, rand(0,130),rand(0,35), rand(0,130), rand(0,35),imagecolorallocate($captcha, 166,246,251));
	}
	
	#generate a random string of 5 characters
	$string = substr(md5(rand()*time()),0,3);
	...
	for($i=0;$i<5;$i++){
		...
		if(file_exists($font)){
			$x=4+$i*23+rand(0,6);
			$y=rand(18,28);
			imagettftext  ($captcha, 15, rand(-25,25), $x, $y, imagecolorallocate($captcha, 166,246,251), $font, $string[$i]);
		}else{
			$x=5+$i*24+rand(0,6);
			$y=rand(1,18);
			imagestring($captcha, 5, $x, $y, $string[$i], imagecolorallocate($captcha, 166,246,251));
		}
	}
